...
####################################################################################################################################
############ GraphCore IPU Code & Model Structure
...
with ipu.ipu_strategy.IPUStrategy().scope():
    early_stopping = keras.callbacks.EarlyStopping(monitor='mean_squared_error', patience=25)
    model_checkpoint = keras.callbacks.ModelCheckpoint("checkpoints/{epoch:04d}.hdf5", save_weights_only=True)
    ...
    history = model.fit(x=X_train, y=y_train, verbose=1, validation_data=(X_test, y_test), batch_size=BATCH_SIZE, epochs=MAX_EPOCHS, callbacks=callbacks)
